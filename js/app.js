$(document).ready(function() {
  ...
  var PokemonArray = function() {
      var arr = [];
      arr.push = function(callback, arguments) {
          // even checken of alle items in de array van type Pokemon zijn
          return callback(Array.prototype.push.apply(this, arguments));
      };
      arr.splice = function (callback, arguments){
        var candyReturned = arr[arguments].candy;
          callback(Array.prototype.splice.apply(this, arguments));
      return candyReturned
      };
      return arr;
  };

  var selectedPokemon =  new PokemonArray();
    ...
    $.ui.autocomplete.filter = function(array, term) {
        ...
        return $.grep(array, function(value) {
            return matcher.test(value.species || value.value || value);
        });
    };
    ...
    $('#pokemonSelect').on('autocompleteselect', function(e, ui) {
      ...
      resetForm();
      selectedPokemon.push(ui.item);
      $('#pokemonChoice').prepend('<h2>You chose <span class="species-instance">' + selectedPokemon[0].species + '! </span></h2><p id="pokemon-desc">Unfortunately <span class="species-instance">' + selectedPokemon[0].species + '</span> does not evolve. :(</p>'); //Inform user
          $('#pokemonSelect').val('');
        ...
        $pokemonAvatar = function() {
            $("#pokemonFields").append('<li class="pokemon-avatar ' + selectedPokemon + '">' + '</li>');
